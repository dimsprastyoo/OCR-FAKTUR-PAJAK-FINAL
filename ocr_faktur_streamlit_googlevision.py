...
st.set_page_config(page_title="OCR Faktur Pajak (Google Vision)", layout="centered")

st.title("📄 OCR Faktur Pajak (PDF) via Google Vision OCR")
st.write("Upload PDF faktur pajak (hasil scan), dan hasilnya akan diubah menjadi Excel.")
...
# Upload PDF
st.subheader("2. Upload file PDF scan faktur")
pdf_files = st.file_uploader("Upload satu atau lebih PDF", type=["pdf"], accept_multiple_files=True)
...
if cred_file and pdf_files:
    # Simpan credential JSON ke file sementara
    ...
    if all_data:
        ...
        st.success("✅ Faktur berhasil dibaca dan diparsing.")
        st.dataframe(df)
